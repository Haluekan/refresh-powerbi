...
import logging
import os
import time # <--- ✅ เพิ่ม time สำหรับระบบ Retry
...
# 2. ตั้งค่า Path
# ----------------------------------------------------------------------
log_file_path = r"D:\OneDrive - Datamars SA\General - IT (Datamars Thailand)\8. Power Platform\Power BI\DATA\export data\logfile\QCOrderlog.log"
output_file_path = r"D:\OneDrive - Datamars SA\General - IT (Datamars Thailand)\8. Power Platform\Power BI\DATA\export data\QC Order 2025.xlsx"
...
data_to_export = [
    {
        "view_name": "vw_QualityOrder_2025",
        "sheet_name": "vw_QualityOrder_2025"
    },
    {
        "view_name": "vw_QualityResult_2025",
        "sheet_name": "vw_QualityResult_2025"
    }
]
...
# ----------------------------------------------------------------------
logging.info(f"--- [!] เริ่มการทำงาน (โหมด: Retry + Multi-Sheet Excel) ---")
...
for attempt in range(1, MAX_RETRIES + 1):
    try:
        ...
        with pyodbc.connect(connection_string) as connection:
            ...
            # เปิด ExcelWriter เพื่อเตรียมเขียน
            # สังเกต: เราต้องเปิด writer ในทุกรอบ Retry เพราะถ้า Error ไฟล์อาจจะเสียหาย
            with pd.ExcelWriter(output_file_path, engine='xlsxwriter') as writer:
                ...
                for item in data_to_export:
                    ...
                    logging.info(f"กำลังดึงข้อมูลจาก View: {view_name} (Chunking)...")
                    ...
                    if total_rows > 0:
                        ...
                        logging.info(f"เขียนชีต '{sheet_name}' สำเร็จ ({total_rows} แถว)")
                    else:
                ...
                if any_data_written:
                    ...
                    logging.warning(f"⚠️ ไม่มีข้อมูลเลย ไฟล์ Excel อาจว่างเปล่า")
        ...
        # ตรวจสอบ Protocol Error หรือ Driver Error
        if 'HY000' in str(e):
